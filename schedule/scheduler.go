...
// runTask 在独立 goroutine 中执行任务（WaitGroup 保护）。
func (s *DefaultScheduler) runTask(st *scheduledTask) {
	defer s.wg.Done()
...
func (s *DefaultScheduler) waitAndClose(ch chan struct{}) {
	s.wg.Wait()
	close(ch)
}
