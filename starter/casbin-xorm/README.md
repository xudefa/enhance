...
Casbin + XORM 集成模块，提供基于数据库的权限策略持久化支持。
...
```go
import (
    "github.com/xudefa/enhance/starter/casbin-xorm"
)
```
...
func main() {
    app, _ := boot.NewApplication(
        boot.WithAppName("casbin-xorm-demo"),
    )
    defer app.Stop()
    
    // 获取 Enforcer
    enforcer := core.MustGetBean[*casbin_xorm.CasbinEnforcer](app.Container())
...
| `casbin.adapter` | string | xorm | 适配器类型 |
...
- **触发条件**: `casbin.enabled=true` 且 `db.xorm.enabled=true`
