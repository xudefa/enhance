...
func (c *CasbinXormAutoConfiguration) Configure(ctx boot.ApplicationContext) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create Casbin XORM adapter: %w", err)
	}
	...
	voter, err := security.NewCasbinVoter(xormEnforcer)
	if err != nil {
...
func (c *CasbinXormAutoConfiguration) startAutoReload(enforcer *XormCasbinEnforcer, cfg *CasbinXormConfig) {
	...
	go c.policyReloadLoop(enforcer, interval)
}
...
// policyReloadLoop 定时重载策略的循环。
func (c *CasbinXormAutoConfiguration) policyReloadLoop(enforcer *XormCasbinEnforcer, interval int) {
