// Package casbinxorm 提供 Casbin 与 XORM 的集成能力。
//
// 本模块是 enhance 框架的扩展模块，将 Casbin 权限管理与 XORM 数据库访问能力集成，
// 支持将 Casbin 策略持久化到数据库中。
//
// # 模块独立性
//
// 本模块采用独立模块设计（拥有独立的 go.mod），依赖隔离确保：
//   - 用户只使用 enhance 核心模块时，不会引入 casbin-xorm 依赖
//   - 用户显式引入本模块时，才会下载 casbin-xorm 及其间接依赖
//   - 避免依赖污染，保持用户项目的依赖树清晰
//
// # 架构设计
//
// 核心组件：
//   - CasbinXormAutoConfiguration: 自动配置类，根据配置文件创建和注册基于 XORM 的 CasbinEnforcer
//
// 自动配置：
//   - 当 security.enabled=true 且 security.casbin.enabled=true 且 security.casbin.policy-type=xorm 时自动生效
//   - 自动创建基于 XORM 的 Casbin 适配器并注册到 IoC 容器
//   - 支持数据库策略持久化
//
// # 快速开始
//
// 1. 在配置文件中启用 Casbin 和 XORM：
//
//	{
//	  "security": {
//	    "enabled": true,
...
//   - security.casbin.policy-type: 策略加载方式，设置为 xorm 时使用数据库存储
...
//   - github.com/casbin/xorm-adapter/v2: Casbin XORM 适配器
