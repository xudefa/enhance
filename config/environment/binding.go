...
func (e *Environment) parsePlaceholder(val string, startIdx int) (key, defaultValue string, hasDefault bool, endIdx int, ok bool) {
	depth := 1
	cursor := startIdx + 2
	for cursor < len(val) && depth > 0 {
		if val[cursor] == '$' && cursor+1 < len(val) && val[cursor+1] == '{' {
			depth++
			cursor += 2
			continue
		}
	...
	if keyEnd >= 0 {
		return inner[:keyEnd], inner[keyEnd+1:], true, cursor, true
	}
	return inner, "", false, cursor, true
}
...
func (e *Environment) resolvePlaceholders(val string, resolving map[string]bool) string {
	...
	pos := 0
	for pos < len(val) {
		if val[pos] == '$' && pos+1 < len(val) && val[pos+1] == '{' {
			key, defaultVal, hasDefault, endPos, ok := e.parsePlaceholder(val, pos)
			if !ok {
				builder.WriteByte(val[pos])
				pos++
				continue
			}
			...
			if resolving[key] {
				...
				builder.WriteString(val[pos : endPos+1])
				pos = endPos + 1
				continue
			}
