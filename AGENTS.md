...
>
> **相关文档**：[代码风格指南](CODING_STYLE.md) | [架构设计](ARCHITECTURE.md) | [贡献指南](CONTRIBUTING.md) | [AI 维护导航](docs/AI_INDEX.md) | [AI 快速参考](docs/AI_QUICK_REF.md)
>
> **AI 首次接入必读**：在动手改代码前，先阅读 [docs/AI_INDEX.md](docs/AI_INDEX.md) 完成代码库导航，并参考 [docs/AI_TASK_HANDBOOK.md](docs/AI_TASK_HANDBOOK.md) 中的任务 SOP。快速查规则/模板见 [docs/AI_QUICK_REF.md](docs/AI_QUICK_REF.md)。

---

### 0. Skill 使用（AI 必读）

- 项目内置 opencode skill：`.opencode/skills/enhance-dev/SKILL.md`。
- AI 首次接入或迭代开发前，应加载该 skill 获取完整工作流（文档导航 → 硬性规范 → 任务 SOP → 验证门）。
- 规则单一来源 `docs/AGENT_RULES.md`，平台文件由 `make skill-sync` 生成，手改平台文件会被拒收。
...
| 函数设计 | ≤ 80 行（不含注释）、≤ 4 参数、嵌套 ≤17 层、时间+空间不能同时 ≥ O(n²) | [CODING_STYLE.md §6](CODING_STYLE.md#6-函数设计) |
...
6. **自检清单**：完成第 5 节的检查清单
7. **运行验证**：执行 `make ai-verify` 和 `make quality-score`
...
6. **检查影响**：使用 `make ai-deps-check` 检查依赖方向
7. **更新文档**：更新相关文档和变更日志
...
- [ ] 函数职责单一，不超过 80 行（不含注释和空行）
- [ ] 嵌套深度不超过 17 层
- [ ] 算法时间+空间复杂度不能同时 ≥ O(n²)
- [ ] 生产文件不超过 500 行，测试文件不超过 1000 行（超过时拆分）
...
#### AI 可维护性
- [ ] 运行 `make ai-verify` 通过
- [ ] 运行 `make quality-score` 评分 ≥ 80
- [ ] 更新相关文档
- [ ] 记录 API 变更
...
- 🚫 修改接口后不更新文档
- 🚫 新增 API 后不记录变更日志
- 🚫 不运行 `make ai-verify` 就提交
- 🚫 不运行 `make quality-score` 就发布
