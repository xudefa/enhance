// Package cache 提供本地缓存实现，采用 LRU 淘汰策略。
//
// TTLCache 是一个实现 Cache 接口的高性能本地缓存。
...
// TTLCache implements a high-performance local cache with LRU eviction.
...
// ttlItem represents a cached item with LRU tracking.
type ttlItem struct {
	key        string
	value      any
	expireAt   time.Time
...
// TTLOption configures the TTL cache.
type TTLOption func(*TTLCache)

// WithTTLMaxSize 设置缓存中的最大条目数。
// 当缓存达到此大小时，最近最少使用的条目将被淘汰。
func WithTTLMaxSize(maxSize int) TTLOption {
	return func(c *TTLCache) {
		c.maxSize = maxSize
	}
}

// WithTTLDefaultTTL 设置缓存条目的默认 TTL。
// 当调用 Set() 且 ttl <= 0 时使用此 TTL。
func WithTTLDefaultTTL(ttl time.Duration) TTLOption {
	return func(c *TTLCache) {
		c.defaultTTL = ttl
	}
}

// NewTTLCache 创建一个新的本地缓存，采用 LRU 淘汰策略。
//
// 该缓存使用 map 实现 O(1) 查找，使用双向链表实现 O(1) LRU 跟踪。默认配置：
//   - 最大容量：1000 个条目
//   - 默认 TTL：5 分钟
func NewTTLCache(opts ...TTLOption) *TTLCache {
	c := &TTLCache{
		maxSize:    1000,
...
func (c *TTLCache) Get(ctx context.Context, key string) (any, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TTLCache) Set(ctx context.Context, key string, value any, ttl time.Duration) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TTLCache) Del(ctx context.Context, keys ...string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Exists 检查键是否存在且未过期。
// 检查期间会清理过期条目。
func (c *TTLCache) Exists(ctx context.Context, key string) (bool, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TTLCache) TTL(ctx context.Context, key string) (time.Duration, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// 调用者必须持有写锁。
func (c *TTLCache) deleteItem(item *ttlItem) {
