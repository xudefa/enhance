...
func TestPathNormalizer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			normalizedPath := normalizer.NormalizePath(tt.input)
			if normalizedPath != tt.expected {
				t.Errorf("NormalizePath(%q) = %q, want %q", tt.input, normalizedPath, tt.expected)
			}
		})
	}
}
...
func TestJoinPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			joinedPath := JoinPath(tt.base, tt.path)
			if joinedPath != tt.expected {
				t.Errorf("JoinPath(%q, %q) = %q, want %q", tt.base, tt.path, joinedPath, tt.expected)
			}
		})
	}
}
