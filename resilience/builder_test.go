...
func TestRandomSelector_Select(t *testing.T) {
	...
	if instance.ID != "instance-1" && instance.ID != "instance-2" {
		t.Errorf("expected ID 'instance-1' or 'instance-2', got %s", instance.ID)
	}
}
...
func TestWeightedSelector_Select(t *testing.T) {
	...
	instances := []InstanceInfo{
		{ID: "instance-1", Weight: 10},
		{ID: "instance-2", Weight: 20},
	}
