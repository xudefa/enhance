...
func analyzeDocumentation(f *fileInfo) []Finding {
	...
	for _, decl := range f.file.Decls {
		...
		case *ast.FuncDecl:
			analyzeFuncDoc(f, declNode, &findings)
		case *ast.GenDecl:
			analyzeGroupDoc(f, declNode, &findings)
		}
	}
	return findings
}
...
func analyzeGroupDoc(f *fileInfo, d *ast.GenDecl, findings *[]Finding) {
	...
	for _, spec := range d.Specs {
		...
		case *ast.TypeSpec:
			if specNode.Name.IsExported() && specNode.Doc == nil && groupDoc == nil {
				*findings = append(*findings, docFinding(f, specNode.Pos(),
					"导出类型 "+specNode.Name.Name+" 缺少 doc 注释",
					"在类型上方添加 // "+specNode.Name.Name+" 描述"))
			}
		case *ast.ValueSpec:
			for _, name := range specNode.Names {
				if name.IsExported() && specNode.Doc == nil && groupDoc == nil {
