...
func TestGetFieldValue_ValidField(t *testing.T) {
	...
	user := User{Name: "John"}
	got := validator.getFieldValue(user, "Name")
	if got != "John" {
...
func TestGetFieldValue_InvalidField(t *testing.T) {
	...
	user := User{Name: "John"}
	got := validator.getFieldValue(user, "NonExistent")
	if got != nil {
		t.Errorf("expected nil for non-existent field, got %v", got)
	}
}
...
func TestGetFieldValue_Pointer(t *testing.T) {
	...
	got := validator.getFieldValue(user, "Name")
	if got != "Jane" {
		t.Errorf("expected 'Jane', got %v", got)
	}
}
...
func TestInterfaceOrNil_CannotInterface(t *testing.T) {
	...
	target := internal{unexported: "secret"}
	v := reflect.ValueOf(target).FieldByName("unexported")
	got := interfaceOrNil(v)
	if got != nil {
