...
func (c *defaultContainer) Get(typ reflect.Type) ([]any, error) {
	...
	for _, beanID := range beanIDsCopy {
		...
		if err != nil {
			return nil, fmt.Errorf("resolve bean %s: %w", beanID, err)
		}
...
func (c *defaultContainer) createAndInitialize(beanID string, def *registry.BeanDef) (any, error) {
	...
	if err := c.checkFactoryCycle(beanID); err != nil {
		return nil, fmt.Errorf("check factory cycle for %s: %w", beanID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resolve scope instance for %s: %w", beanID, err)
	}

	// 调用初始化回调（支持函数式回调和 LifecycleBean 接口）
	if err := c.postCreateBean(beanID, def, instance); err != nil {
		return nil, fmt.Errorf("post create bean %s: %w", beanID, err)
	}
