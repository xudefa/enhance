...
func (c *SecurityAutoConfig) Configure(ctx boot.ApplicationContext) error {
	...
	if err := env.BindPrefix("db.xorm", &xormCfg); err == nil {
		fmt.Printf("[XORM] 配置: enabled=%v, type=%s, host=%s:%d, database=%s, show-sql=%v\n",
			xormCfg.Enabled, xormCfg.Type, xormCfg.Host, xormCfg.Port, xormCfg.Database, xormCfg.ShowSQL)
	}
