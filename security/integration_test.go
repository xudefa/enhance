...
func TestPasswordEncoding(t *testing.T) {
	...
	sha256Encoded := sha256.Encode("password123")
	if sha256Encoded == "password123" {
		t.Error("SHA256 encoder should hash the password")
	}
	if !sha256.Matches("password123", sha256Encoded) {
		t.Error("SHA256 encoder should match")
	}
...
func TestMultipleProviderManager(t *testing.T) {
	...
	daoProvider := NewDaoAuthenticationProvider(userDetailsService, passwordEncoder, log.Build())
