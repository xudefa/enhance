...
// 职责：将标准库的http.Request适配为SecurityRequest接口
// 使安全模块能够透明地访问HTTP请求信息
...
// 职责：将标准库的http.ResponseWriter适配为SecurityResponse接口
// 使安全模块能够透明地写入HTTP响应
...
// 职责：将SecurityFilterChain适配为标准库的http.Handler
// 执行流程：
// 1. 接收HTTP请求，适配为SecurityRequest和SecurityResponse
// 2. 执行安全过滤器链（认证、授权等）
// 3. 如果过滤器链执行成功且状态码正常，将认证信息注入request context
// 4. 调用下一个handler（业务处理器）
...
// 职责：从Authorization头中提取Basic认证信息并进行认证
// 认证流程：
// 1. 检查Authorization头是否存在且为Basic类型
// 2. 解码Base64获取用户名和密码
// 3. 调用AuthenticationManager进行认证
// 4. 认证成功后将认证信息设置到全局安全上下文和request context
...
func (f *BasicAuthenticationFilter) DoFilter(ctx context.Context, request SecurityRequest, response SecurityResponse, chain SecurityFilterChain) error {
	...
	// 如果不是 Basic 认证，跳过（可能是 Bearer Token 等其他认证方式）
	const prefix = "Basic "
