...
func TestDefaultExceptionResolver_Resolve(t *testing.T) {
	...
	if resp == nil || resp.Code != 500 || resp.Message != "Internal Server Error" {
		t.Fatalf("Expected code 500 and message 'Internal Server Error', got code=%v, message=%v", resp != nil, resp != nil)
	}
}
